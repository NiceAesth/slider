...
class Circle(HitObject):
    ...
    def _parse(cls, position, time, hitsound, new_combo, combo_skip, rest):
        ...
        return cls(
            position, time, hitsound, *rest, new_combo=new_combo, combo_skip=combo_skip
        )
...
def _invalid_to_default(
    field: str, field_value, expected_type, default: any = no_default
):
    """
...
def _pack_str_list(
    field: str, list_str: list, sep: str = " ", default: any = no_default
):
...
def _pack_timedelta_list(
    field: str, list_td: list, sep: str = ",", default: any = no_default
):
...
class Beatmap:
    ...
    colours: dict[str, tuple[int, int, int]]
        The combo colours in the map, stored as a mapping from combo index to
        RGB values.
    ...
    def parse(cls, data):
        ...
        if "Events" in groups:
            for line in groups["Events"]:
                if (
                    line.startswith("0") and background is None
        ...
        return cls(
            ...
            colours={
                k: tuple(int(e.strip()) for e in v.split(","))
                for k, v in (line.split(":", 1) for line in groups.get("Colours", []))
            },
        )
    ...
    def _calculate_stars(self, easy, hard_rock, speed_scale):
